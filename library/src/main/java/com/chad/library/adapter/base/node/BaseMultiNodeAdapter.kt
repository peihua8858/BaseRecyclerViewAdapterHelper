...
class BaseMultiNodeAdapter<T : BaseNode<T>>(nodeList: MutableList<T>? = null)   : BaseProviderMultiAdapter<T>(null) {
    ...
    override fun isFixedViewType(type: Int): Boolean {
        return fullSpanNodeTypeSet.contains(type) || super.isFixedViewType(type)
    }
     ...
     * @param layoutResId
     */
    protected fun addFullSpanItemType(type: Int, @LayoutRes layoutResId: Int) {
        fullSpanNodeTypeSet.add(type)
        layouts.put(type, layoutResId)
    }
     ...
     */
    fun addNodeProvider(provider: BaseNodeProvider<T>) {
     ...
     * @param provider BaseItemProvider
     */
    fun addFullSpanNodeProvider(provider: BaseNodeProvider<T>) {
     ...
     * @param provider BaseItemProvider
     */
    fun addFooterNodeProvider(provider: BaseNodeProvider<T>) {
     ...
     */
    override fun addItemProvider(provider: BaseItemProvider<T>) {
    ...
    private fun removeChildAt(position: Int): Int {
        ...
        if (!node.childNode.isNullOrEmpty()) {
            if (node is BaseExpandNode<*>) {
                if (node.isExpanded) {
                    val items = flatData(node.childNode as Collection<T>)
...
//            notifyItemRangeChanged(parentIndex + 1 + getHeaderLayoutCount(), max(removeCount, newFlatData.size))
            ...
            if (element is BaseExpandNode<*>) {
                ...
                if (isExpanded == true || element.isExpanded) {
                    ...
                    if (!childNode.isNullOrEmpty()) {
                        val items = flatData(childNode as Collection<T>, isExpanded)
     ...
     */
    fun findParentNode(node: BaseNode<T>): Int {
    ...
    override fun bindClick(viewHolder: BaseViewHolder) {
        bindItemClickListener(viewHolder)
        bindItemLongClickListener(viewHolder)
    }

    private fun bindItemLongClickListener(viewHolder: BaseViewHolder) {
        val l = getOnItemLongClickListener()
        val itemViewType = viewHolder.itemViewType
        val provider = getItemProvider(itemViewType)
    ...
    private fun bindItemClickListener(viewHolder: BaseViewHolder) {
        val l = getOnItemClickListener()
        val itemViewType = viewHolder.itemViewType
        val provider = getItemProvider(itemViewType)
        if (l == null && provider != null) {
            //如果没有设置点击监听，则回调给 itemProvider
            //Callback to itemProvider if no click listener is set
            viewHolder.itemView.setOnNoDoubleClickListener {
                var position = viewHolder.adapterPosition
                if (position == RecyclerView.NO_POSITION) {
            ...
            viewHolder.itemView.setOnNoDoubleClickListener { v ->
                var position = viewHolder.adapterPosition
                if (position == RecyclerView.NO_POSITION) {
                    return@setOnNoDoubleClickListener
                }
    ...
    override fun bindChildClick(viewHolder: BaseViewHolder, viewType: Int) {
        bindItemChildClickListener(viewHolder)
        bindItemChildLongClickListener(viewHolder)
    }

    private fun bindItemChildLongClickListener(viewHolder: BaseViewHolder) {
        val l = getOnItemChildLongClickListener()
        val itemViewType = viewHolder.itemViewType
        val provider = getItemProvider(itemViewType)
        if (l == null && provider != null) {
    ...
    fun bindItemChildClickListener(viewHolder: BaseViewHolder) {
        val l = getOnItemChildClickListener()
        val itemViewType = viewHolder.itemViewType
        val provider = getItemProvider(itemViewType)
        if (l == null && provider != null) {
            ...
            ids.forEach { id ->
                viewHolder.itemView.findViewById<View>(id)?.let {
                    ...
                    it.setOnNoDoubleClickListener { v ->
                        var position: Int = viewHolder.adapterPosition
                        if (position == RecyclerView.NO_POSITION) {
                            return@setOnNoDoubleClickListener
                        }
            ...
            for (id in getChildClickViewIds()) {
                viewHolder.itemView.findViewById<View>(id)?.let { childView ->
                    ...
                    childView.setOnNoDoubleClickListener { v ->
                        var position = viewHolder.adapterPosition
                        if (position == RecyclerView.NO_POSITION) {
                            return@setOnNoDoubleClickListener
                        }
